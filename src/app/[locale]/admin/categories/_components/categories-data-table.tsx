            ...
            <div className='flex items-center justify-between p-4 border-b border-gray-200 dark:border-black/60 bg-white dark:bg-black/90 shadow-sm'>
                ...
                <div className='flex items-center gap-4 min-w-[300px] max-w-[500px]'>
                    ...
                    <Tooltip>
                        ...
                        <TooltipContent side='bottom'>{t('addCategory')}</TooltipContent>
                    </Tooltip>
                </div>
            </div>
                        ...
                        {table.getHeaderGroups().map((headerGroup) => (
                            <TableRow key={headerGroup.id} className='bg-gray-50 dark:bg-white/5'>
                                {headerGroup.headers.map((header) => (
                                    <TableHead
                                        key={header.id}
                                        className='px-4 py-2 text-left font-medium text-gray-800 dark:text-gray-100 text-sm border-b border-gray-200 dark:border-gray-600'
                                    >
                    ...
                    <TableBody>
                        {table.getRowModel().rows.length ? (
                            table.getRowModel().rows.map((row) => (
                                <TableRow key={row.id} className='border-t hover:bg-gray-50 dark:hover:bg-white/10'>
