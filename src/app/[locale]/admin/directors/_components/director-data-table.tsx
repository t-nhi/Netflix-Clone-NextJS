...
import { useDeleteDirectorMutation, useGetAllDirectorsQuery } from '@/store/services/director/director.services'
import { getDirectorColumns } from '@/app/[locale]/admin/directors/_components/director-columns'

export default function DirectorsDataTable() {
    const t = useTranslations('AdminPage.directorPage')
    const { data: getDirestorsRes } = useGetAllDirectorsQuery()
    const directors = useMemo(() => getDirestorsRes?.data || [], [getDirestorsRes])
    ...
    const [deleteDirectorMutate] = useDeleteDirectorMutation()

    const onEdit = (id: string) => {
        router.push(`${AdminPaths.DIRECTORS}/${id}/edit`)
    }

    const onDelete = async (id: string) => {
        try {
            await deleteDirectorMutate({ params: { id } }).unwrap()
    ...
    const columns = getDirectorColumns({ t, onEdit, onDelete })
                ...
                <div className='flex items-center gap-4 min-w-[300px] max-w-[500px]'>
                    ...
                    <Tooltip>
                        <TooltipTrigger asChild>
                            <Button
                                onClick={() => router.push(`${AdminPaths.DIRECTORS}/add`)}
                        ...
                        <TooltipContent side='bottom'>{t('addDirector')}</TooltipContent>
                    </Tooltip>
                </div>
            </div>
